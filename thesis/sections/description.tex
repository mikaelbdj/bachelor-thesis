...
\end{theorem}
\begin{proof}
Here, we charge each step to a node. We prove that we charge no more than $2 log\n +3$ steps per node, which gives the bound of $2|V|log|V|+3|V|$.
\end{proof}
...
\begin{theorem}\label{linear-strengthened} Linear($V, E, \pair{\emptyset}{\emptyset}$) runs in $O(|V|)$ symbolic steps.
\end{theorem}
\todo{add corollary 1 ?}
